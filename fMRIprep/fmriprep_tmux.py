...
python fmriprep_tmux.py [main directory] [bids directory] [deriv directory] [temp directory]
						[subject] [processessors] [use aroma]
-----------------------------------------------------------------------------------------
...
# run singularity
singularity_cmd = "singularity run --bind /{main_dir}:/{main_dir} {dir} {source} {deriv_dir} participant --participant_label {sub} -w {temp} --output-space T1w template fsaverage --nthreads {nb_thread:.0f} --use-syn-sdc --low-mem {ica_aroma} --fs-license-file {fs_licence}".format(  
									main_dir = main_dir,
									dir =singularity_dir, 
									source = bids_dir, 
									deriv_dir = deriv_dir,
